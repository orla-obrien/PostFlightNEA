...
subtitle: Southern New England Wind Energy Areas and Surrounding Waters
header-includes:
...
```{r DMA/SMA statement, echo=FALSE, warning = FALSE, message = FALSE}


DMA_statement <- "Active DMAs or Slow Zones in yellow. Active SMAs in red."

SMA_statement <- if (dat$month[1] == 11 ) {
    paste("Active SMAs in red.") 
...
```{r Survey Type statement, echo=FALSE, warning = FALSE, message = FALSE}
SurveyTypeStatement <- if (SurveyType == "G" ) {
    paste("general survey") 
  }else if (SurveyType == "D" ) {
    paste("directed survey")  
  }else if (SurveyType == "C" ) {
    paste("condensed survey") 
  }
```
...
On `r surveyday`, `r text.date.short`, the New England Aquarium flew `r trackno` `r SurveyTypeStatement` tracklines. Weather `r wx_pf`; Beaufort sea state `r beauf`. `r egs` `r eg_statement` sighted during this flight. Marine mammal and turtle sighting details are listed in the table(s) below.
...
rw_sight_table <- dat %>%
  filter(speccode == "RIWH") %>%
  dplyr::select(month, day, year, time, lat, long, number, b1) %>%
  unite(Date, month, day, year, sep = "/") %>%
  rename(Date = Date, Time = time, 
         Latitude = lat, Longitude = long,
         Number = number, Behavior = b1)

colnames(rw_sight_table) <- c("Date", "Time", "Latitude", "Longitude", "Number of individuals", "Behavior") 

if (egnum > 0) {
    kbl(rw_sight_table, booktabs = T, caption = "Right Whale Sightings Details", linesep = "") %>%
  kable_styling(latex_options = c("hold_position", "repeat_header"), full_width = F, font_size = 9)

}

# if (egnum > 0) {
#     kbl(rw_sight_table, booktabs = T, caption = "Table 2. Right Whale Sightings Details", linesep = "") %>%
#   kable_classic(full_width = F, font_size = 12)
#   
# } 
